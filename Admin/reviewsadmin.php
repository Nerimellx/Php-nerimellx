...
$action = clear_string($_GET['action']);
   ...
   switch ($action) {
        ...
        case 'delete':     
        $delete = mysqli_query($connection, "DELETE FROM `coment` WHERE `id` = '$id'");        
        break;
        
    }
if (isset($_POST['correctcom']))
{
...
$coment = mysqli_query($connection, "SELECT * FROM `coment`");
					while($com = mysqli_fetch_assoc($coment) )
					{
						...
						<div class="col-md-9">
							...
							<form method="POST">
							<label>Id: </label><input name="id" value="'.$com['id'].'"></input>
							<label>Имя: </label><input name="name" value="'.$com['name'].'"></input>
							<label>Текст: </label><input name="text" value="'.$com['text'].'"></input>
							<input type="submit" name="correctcom" value="Изменить">
					</form>
							</div>
						</div>
						<div class="col-md-2 comment-data">
							'.$com['pubdates'].'
							<div class="delete-cart"><a href="reviewsadmin.php?id='.$com["id"].'&action=delete"><i class="fa fa-trash-o" aria-hidden="true"></i></a></div>
						</div>
					</div>';
					}
					echo '
